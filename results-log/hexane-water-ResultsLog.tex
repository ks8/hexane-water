...
\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{0_0-32bead-bonded-tip4pF-235-bin01-long}
\end{figure}

\item Paper review: Freezing water in no-man's land, A. Manka, H. Pathak, S. Tanimura, J. Wolk, R. Strey, and B. E. Wyslouzil, 2012.  
\subitem Report homogeneous ice nucleation rate between 202 K and 215 K.  These temperatures are well below the homogeneous freezing limit 235 K for bulk water, and well within no-man's land.  Experimental methods to study highly supercooled liquid water rely on decreasing the size of the sample.  Key idea is that reducing sample size and increasing cooling rate, probability of nucleation is decreased and sample can be pushed further into the metastable region.  Nanoscale droplets thus have a lower boundary of homogeneous freezing limit.  KEY TAKEAWAY: the experimental freezing limit for micrometer scale water droplets is around 235 K.  
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{alkane_water_orientations}
\end{figure}
Monitoring the influence of H-bonds on particular interfacial water molecules allows different H-bonding environments to be identified.  Figure 5 shows orientational distributions, which can explain the VSF response.  On average, the free OH is oriented out of the interface (about 30 degrees from normal) with very little cancelation.  The donor OH bonds however are oriented roughly parallel to the interface, pointing only slightly into the bulk water phase.  Although the number of donor and free OH bonds is the same, the susceptibility arising from the donor OH is considerably smaller; this is attributed to the largely in-plane nature of the donor OH bonds.  
\item KEY THOUGHTS: They may not be completely accurately reproducing VSF from their simulations, or vice versa they got lucky with parameter choices.  ALSO, they are only discussing idk.......
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{pressure_block_averaging_test}
\end{figure}
Now running pressure\_analysis\_UPDATE.py -pressure\_file=pressure\_scalar-press-198-1bead.txt -index\_start=1 -index\_end=6000000 -num\_SE\_blocks= 100.  So, using 100 blocks of size 30 ps, we get an average pressure of -69.85286411 with a standard error of 0.75648606.  Now downloading the tensor data, pressure\_tensors-press-298-1bead.txt.  Result is around 40, with a SE of 30 -- so it appears we do need more independent runs for smaller SE value, as with former NPT runs.  So, what gives with negative pressure?  Now, to check this average pressure, we are running filename press-298-1bead-restart1, 0 equil NVT steps, 6M production NVT steps at 298 K, no beads, press every 1, waldman, restart/traj every 500,000, no z change, restart from press-298-1bead\_6000000.xml.   
\item Checking out Mike's new RDF calculator.  in project directory, for\_kirk\_rdf/NEW\_RDF.  from hexane-water/dash\_work/water/RDF-new/, copying cmd\_read\_subs.o and cmd\_read\_subs.mod.  then ran bash compile.sh.  Also copying sys.RDF-tip4pF-32bead-1000.data.txt.  Now we want to generate just a few snapshots from a 32bead 1000 water molecule simulation.    Using run3.sh in dash\_work/water, we run filename RDF-NEW, 1000 waters, 50,000 steps, NVT at 235 K, initial 0.983 density, 32 beads, record every 10,000 steps.  Taking the resulting RDF-NEW-10000.xyz file and copying to for kirk folder.  Ok, Mike made changes which now allow the program to work.  Trying this on the interfacial system.  We should have the restart file full-235-32bead-part21800000.xml.  Let's use this to check just a few frames, say 10, for the O-O RDF.  filename RDF-235-32bead-interface, 1000 steps, recording every 100 steps.  Copying RDF-235-32bead-interface.xyz and RDF-235-32bead-interface\_1801000.xml to /project/depablo/kswanson/for\_kirk\_rdf/NEW-RDF.  
...
\subitem 32 beads: 217.42 s
\subitem 16 beads: 126.18 s
\subitem 8 beads: 79.64 s
\subitem 4 beads: 52.63 s
\subitem 1 bead: 28.966 s
for DASH
\subitem 32 beads: 241.90 s
\subitem 16 beads: 138.62 s
\subitem 8 beads: 82.39 s
\subitem 4 beads: 52.20 s
\subitem 1 bead: 65.46 s

for 40,000 steps:

\subitem On Midway-l13b-23 for OpenMM
\subitem 32 beads: 409.06 s
\subitem 16 beads: 228.54 s
\subitem 8 beads: 138.12 s
\subitem 4 beads: 83.84 s
\subitem 1 bead: 34.63 s
for DASH
\subitem 32 beads: 473.8
\subitem 16 beads: 278.28
\subitem 8 beads: 160.45 s
\subitem 4 beads: 105.03 s
\subitem 1 bead: 128.14 s
...
OpenMM:
\subitem 32 beads: 193.88 s
\subitem 16 beads: 104.34 s
\subitem 8 beads: 60.15 s
\subitem 4 beads: 32.64 s
\subitem 1 bead: 7.00 s

DASH:
\subitem 32 beads: 129.16 s
\subitem 16 beads: 86.98 s
\subitem 8 beads: 53.29 s
\subitem 4 beads: 37.8 s
\subitem 1 bead: 61.16 s
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{intrinsic-width-32bead-vs-1bead-7-26-2018-points}
\end{figure}

\item From Markland: The melting temperature of the q-TIP4P/F model obtained from our coexistence simulations of the ice-water interface was found to be 251 +/- K, around 22 K lower than the experimental value.  The larger error bars associated with this value are a result of the slower motion at such low temperatures, which increases the time scale required to observe melting or ice lattice formation at the interface.  Earlier: The melting point calculations...were done by performing direct coexistence simulations of the water-ice interface under atmospheric pressure.  Initial hexagonal ice configurations were generated by placing the oxygen atoms at their crystallographic sites...H atoms placed so that the total dipole moment of the simulation cell was exactly zero.  The initial ice configuration was then equilibrated in the presence of an Andersent thermostat and isotropic Berendsen barostat before placing the secondary prismatic face of the ice cell in contact with an equilibrated water simulation.  360 molecules in ice phase, 336 in liquid phase.  Monitored extent of melting or freezing.  PERHAPS DO MUCH LONGER SIMULATIONS AT LOWER TEMPERATURES TO SHOW lower error bars?
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{pressures_lammps}
\end{figure}
Now, we will check the average pressure in a simple block of water, first on DASH, then in LAMMPS.  We will simulate 500 water molecules, 2M steps, NVT 298 K, 0.997 initial density, no PI, no recording of restart/traj, filename press-NVT-1bead-water.    

\item C-C hexane RDF: 

\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{C_C-1bead-bonded-TAFFI-hexane-298-bin01}
\end{figure}
In /project/depablo/kswanson/hexane-water/dash\_work/interface, we use run\_RDF.sh to do run hexane-RDF.py, which will do 200,000 .5 fs steps at 298 K, NVT, recording every 1000 steps.  filename hexane-RDF-1bead-long.  We have, using bin 0.01:
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{H_H-1bead-bonded-TAFFI-hexane-298-bin01-long}
\end{figure}

Now that we have these results, let's do the same for OPLS in LAMMPS for comparison.  Going into /project/depablo/kswanson/hexane-water/lammps\_work/hexane and using in.hexane.   Ran 0.5 timestep, NPT at 298 K 1 atm, 500 hexanes, 200,000 steps.  Now, restarting from that, 200,000 NVT 298 K steps, filename RDF-OPLS-hexane-1bead-prod.    Running this now.  
\item ERROR IN DASH: atoms.size() is used to compute the the ideal gas contribution to the pressure, which will cause an overestimation of the ideal gas component in DASH.  
\end{enumerate}
...
\end{figure}
Similar to DASH, we see that 5 ps is appropriate so we divide into 198 blocks.  We get mean pressure of 85.206, SE of 2.418, sd of 512.248.   
Here is what the plot looks like:
\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{pressures_press-NVT-1bead-water-lammps}
\end{figure}
\item Now, downloading pressure\_scalar-press-NPT-1bead-water, the 2M NPT run described above. 
\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{pressures_press-NPT-1bead-water}
\end{figure} 
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{pressures_press-NPT-1bead-water-highpress-full}
Using similar 198 blocks, mean of pressure is 95.66, SE is 7, sd is 1274.  
\end{figure} 
\item fulldata-NPT-298 got canceled due to error already set at 2365000 steps, meaning it recorded 2360000 steps and needs 4640000 more steps.  So, restarting from fulldata-NP-298\_2360000.xml, filename fulldata-NP-298-restart1, no z change, restart/traj 10,000, pressure every 10, no PI, 4640000 NVT prod steps, 
\end{enumerate}
