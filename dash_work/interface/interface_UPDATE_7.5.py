...
def create_parser():
    ...
    parser.add_argument('-dataFreq', dest='dataFreq', default=20, help='Frequency at which thermodynamic data are recorded')
...
def process_datafile(files, options):
	...
	if restart:
		...
		# Create pressure tensor file
		output = open('pressure_tensors-'+filename+'.txt','w')
		output.close()

		# Create pressure calar file
		...
		def pressure_recording(currentTurn):

			# Print pressure tensors
			output = open('pressure_tensors-'+filename+'.txt','a')
			output.write(str(dataPressureTensor.vals[-1][0]) + '\t')
			output.write(str(dataPressureTensor.vals[-1][1]) + '\t')
			output.write(str(dataPressureTensor.vals[-1][2]) + '\n')
			output.close()
